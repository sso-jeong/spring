	...
	<div class="login-wrap">
		<div class="login-box">
			<form onsubmit="return checkPwd();" method="post" action="${pageContext.request.contextPath}/grp_register" autocomplete="off">
				...
				</select> <select name="empGradeCode" id="empGradeCode">
				</select> <input type="date" max="2020-12-31" min="1970-01-01" name="empEnter" id="empEnter" placeholder="입사년월일을 입력하세요." required tabindex="3" /> <input type="text" name="empName" id="empName"
					placeholder="사원명을 입력하세요." required tabindex="4" /> <input type="password" name="empPwd" id="empPwd" placeholder="비밀번호를 입력하세요." required tabindex="5" /> <input type="password" name="repasswd"
					id="repasswd" placeholder="비밀번호 확인을 입력하세요." required tabindex="6" />
				<div class="">
...
<script>
	...
	/* function checkPwd() {
		...
		if(a.val().length > 15 || a.val().length < 10){
			alert("비밀번호를 10자 이상 15자 이하로 입력해주세요");
			a.val("");
			a.focus();
			return false;
		}
